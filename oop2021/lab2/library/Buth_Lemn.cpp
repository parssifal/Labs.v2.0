...
namespace Prog2 {
	double ltype(double c, double m) {       //Определяет
		double i;                          //тип кривой
		if (c > 2 * m * m) return i = 0;//elleptic
		if (c < 2 * m * m) return i = 1;//hyperbolic
		if ((c == 0) or (c == 2 * m * m)) return i = -1;
		return 0;
	}

	double parametrs(double c, double m) {          //Определяет
		if (ltype(c, m) == 0) {//e               //параметры
			double a2 = 2 * m * m + c;      //для полярного
			double b2 = 2 * m * m + c;
			cout << "a = " << a2 << endl;   //уравнения
			cout << "b = " << b2 << endl;
		}                                       //зависящие
		else if (ltype(c, m) == 1) {//h          //от типа
			double a22 = 2 * m * m + c;      //кривой
			double b22 = 2 * m * m - c;
			cout << "a = " << a22 << endl;
			cout << "b = " << b22 << endl;
		}
		else if ((c == 0) or ltype(c, m) == -1) {
			cout << "Error! It is not Lemniskata of Buth anymore." << endl;
		}
		return 0;
	}
	...
	char* Prog2::Buth_Lemn::frm(double type) const { //формула лемнискаты Бута
		double i = type;
		if (i == 0) {
